...
If the app crashes while sending, rows stuck as “publishing” are put back to “pending” and tried again. A store test leaves a row in `publishing`, reclaims it, then flushes: Kafka sees the event once.
