...
/** The edge fade a peeking carousel wears, as a CSS mask image. */
const peekMask = (peek: number) =>
...
export function useSnapCarousel({
  ...
  const trackProps = {
    drag: disabled || total < 2 ? false : ("x" as const),
...
export function SnapCarousel({
  ...
  const button =
    "grid size-7 place-items-center rounded-md border border-border bg-card text-foreground shadow-sm outline-none transition-[background-color,border-color,box-shadow,transform] duration-150 hover:bg-muted active:translate-y-px active:shadow-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0";
    ...
    <div className={`w-full ${className}`}>
      <div
        ...
        style={{
          paddingLeft: peek,
          paddingRight: peek,
          // A mask applies only when the carousel peeks at its neighbours.
          // Leaving the property undefined omits it, as the old conditional
          // spread of an empty object did.
          WebkitMaskImage: peek > 0 ? peekMask(peek) : undefined,
          maskImage: peek > 0 ? peekMask(peek) : undefined,
        }}
        className="relative overflow-hidden rounded-xl py-1.5 outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0"
        {...car.viewportProps}
      >
        <motion.div
          {...car.trackProps}
        ...
        <span className="flex items-center gap-[3px]">
          {slides.map((slide, i) => (
            <button
              ...
              className="grid h-[18px] w-[16px] place-items-center rounded-sm outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0"
            >
              <motion.span
                initial={false}
                animate={
                ...
                className="block h-[5px] w-[14px] rounded-[2px] bg-primary"
              />
            </button>
          ))}
        </span>
