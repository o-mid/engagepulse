...
function DeckCard({
  ...
  const badge = (side: -1 | 1, text: string, place: string) => {
    ...
    return (
      <motion.span
        ...
        className={`pointer-events-none absolute top-3 whitespace-nowrap rounded-md border bg-card px-2 py-1 text-[10.5px] font-semibold uppercase tracking-[0.08em] transition-colors duration-150 ${
  ...
  return (
    <motion.div
      ...
      className={`absolute inset-x-5 top-0 select-none overflow-hidden rounded-xl border border-border bg-card text-card-foreground ${
        active
...
export function SwipeDeck<T>({
  ...
  const control =
    "inline-flex h-8 items-center gap-1.5 rounded-md border border-border bg-card px-2.5 text-[12px] font-medium text-foreground outline-none transition-[background-color,border-color,opacity] duration-150 hover:bg-muted focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0";
  ...
  return (
    <div className={`w-full ${className}`}>
      <div
        ...
        className="relative w-full overflow-hidden rounded-xl outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0"
        {...deck.deckProps}
      >
        <div className="absolute inset-0 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_20px,black_calc(100%-20px),transparent)]">
          <motion.div
            ...
            style={{ height }}
            className="absolute inset-x-5 top-0 z-0 grid place-items-center rounded-xl bg-muted px-4 text-center text-[12.5px] text-muted-foreground"
          >
            {emptyLabel}
          </motion.div>
      ...
      <div className="mt-3 grid h-8 grid-cols-[1fr_auto_1fr] items-center gap-3">
        ...
        <span className="flex items-center gap-2 font-mono text-[10.5px] tabular-nums text-muted-foreground">
          ...
          <button
            ...
            onClick={deck.undo}
            inert={!deck.canUndo}
            className={`inline-flex items-center gap-1 rounded-sm px-1 py-0.5 text-foreground outline-none transition-[background-color,box-shadow,opacity] duration-150 hover:bg-muted focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0 ${spent(
              !deck.canUndo,
            )}`}
          >
            {ICON_UNDO}
