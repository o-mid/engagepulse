...
// WizardSteps: multi-step wizard with a numbered tile rail, sliding panels, and focus handoff. Restyled onto the
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      >
        {steps.map((s, i) => {
          ...
          const tile = (
            <motion.span
              aria-hidden
              className={`grid size-7 place-items-center rounded-md border text-[11.5px] font-medium tabular-nums shadow-sm transition-colors duration-150 ${
                done
            ...
            <li key={s.id} className="flex flex-1 items-center gap-1 last:flex-none">
              ...
              {i <= furthest ? (
                <button
                  ...
                  className="rounded-md outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0"
                >
                  {tile}
                </button>
              ) : (
              ...
              {i < total - 1 ? (
                <span
                  aria-hidden
                  className="relative h-[3px] flex-1 overflow-hidden rounded-[2px] bg-muted"
                >
                  <motion.span
      ...
      <div
        ...
        aria-label={position}
        style={{ height }}
        className="relative overflow-hidden rounded-lg border border-border bg-card shadow-sm outline-none transition-[border-color,box-shadow] duration-150 focus-visible:border-ring"
      >
        <AnimatePresence initial={false} custom={direction}>
          <motion.div
            ...
            className="absolute inset-0 overflow-y-auto overscroll-contain p-4 text-[13.5px] leading-relaxed text-card-foreground"
          >
        ...
        <AnimatePresence initial={false}>
          {isFirst ? null : (
            <motion.button
              ...
              className="h-9 rounded-md border border-border bg-card px-3 text-[13px] font-medium text-foreground outline-none transition-[border-color,box-shadow] duration-150 hover:border-foreground/30 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0"
            >
              {backLabel}
            </motion.button>
          )}
        ...
        <AnimatePresence initial={false}>
          {complete ? null : (
            <motion.button
              ...
              transition={reduced ? { duration: 0 } : CROSSFADE}
              className="ml-auto grid h-9 place-items-center rounded-md bg-primary px-3.5 text-[13px] font-medium text-primary-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0"
            >
