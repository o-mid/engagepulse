...
export function Dropdown({
  ...
  return (
    <div ref={rootRef} className={`relative inline-block text-left ${className}`}>
      <button
        {...triggerProps}
        className={`flex h-9 select-none items-center gap-2 whitespace-nowrap rounded-md border border-border bg-card px-3 text-[13px] font-medium text-foreground outline-none transition-[box-shadow,border-color] duration-150 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0 disabled:opacity-50 ${
          open ? "shadow-inner" : "shadow-sm hover:border-foreground/30"
        }`}
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute left-0 top-[calc(100%+6px)] z-50 min-w-[224px] whitespace-nowrap rounded-lg border border-border bg-card p-[5px] text-card-foreground shadow-lg"
          >
            <ul
              {...listProps}
              aria-label={label}
              ...
              {items.map((item, i) => {
                ...
                return (
                  <li
                    key={item.value}
                    {...getItemProps(i)}
                    className={`relative flex h-8 cursor-default select-none items-center rounded-md px-2.5 text-[13px] ${
                      item.disabled
