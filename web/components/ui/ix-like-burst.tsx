...
/**
 * Persists a like. Receives the intended state and an AbortSignal that fires
 * when a newer toggle supersedes this one. Its resolved value is ignored.
 */
export type LikeCommit<TCommitResult = void> = (
  liked: boolean,
  signal: AbortSignal,
  ...
  return (
    <span className={`inline-flex items-center ${className}`}>
      <button
        ...
        className="inline-flex h-9 select-none items-center gap-2 rounded-md border border-border bg-card px-3 text-[13px] font-medium text-card-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0 disabled:opacity-50"
      >
        <span aria-hidden className="relative block size-[18px]">
          ...
          <motion.svg
            viewBox="0 0 24 24"
            fill="currentColor"
            className="absolute inset-0 size-[18px] text-primary"
            initial={false}
          ...
          {!reduced && burst > 0 ? (
            ...
            >
              {SPARKS.map((spark, i) => (
                <motion.span
                  key={i}
                  className="absolute block rounded-[2px] bg-primary"
                  style={{
                    width: spark.size,
                    height: spark.size,
