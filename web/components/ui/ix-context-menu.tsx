...
export function useContextMenu({
  ...
  const triggerProps = {
    ...
    // SAFETY: `WebkitTouchCallout` is a vendor property React passes straight
    // through to the DOM, and CSSProperties does not declare it.
    ...
    onContextMenu: (event: ReactMouseEvent<HTMLElement>) => {
      ...
      event.stopPropagation();
      clearHold();
      // SAFETY: these props are spread onto the trigger div this component
      // renders, so currentTarget is that div on every dispatch.
  ...
  return (
    <>
      <div
        ...
        className={`outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0 ${className}`}
      >
        {children}
      ...
      <AnimatePresence>
        {placement ? (
          <motion.div
            ...
            className="overflow-y-auto overscroll-contain rounded-xl border border-border bg-card p-[5px] text-card-foreground shadow-lg outline-none"
          >
            {items.map((item, index) =>
              item.type === "separator" ? (
                ...
                <button
                  ...
                  {...getItemProps(index)}
                  className={`flex h-[32px] w-full cursor-default select-none items-center gap-2 rounded-md px-2.5 text-left text-[13px] outline-none ${
