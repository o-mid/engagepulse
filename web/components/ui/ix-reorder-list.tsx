...
export function ReorderList<T>({
  ...
  return (
    <div className={`w-full ${className}`}>
      <Reorder.Group
        axis="y"
        // SAFETY: Reorder.Group asks for a mutable array but only reads it;
        // reordering arrives back through onReorder as a new array.
        values={items as T[]}
        onReorder={onReorder}
        aria-label={label}
        ...
        {items.map((item) => {
          ...
          return (
            <Reorder.Item
              ...
              className={`relative flex items-center gap-2.5 rounded-md border bg-card px-3 py-2.5 outline-none transition-[border-color,box-shadow,background-color] duration-150 focus-visible:outline-none ${
                lifted
                  ? "z-10 cursor-grabbing border-border shadow-lg"
                  : "cursor-grab border-border shadow-sm"
              } ${
                held
                  ? "border-ring bg-[var(--ring)]/10"
