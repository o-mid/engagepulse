...
export function useReadingProgress({
  ...
  useEffect(() => {
    ...
    const observer =
      !("ResizeObserver" in globalThis) ? null : new ResizeObserver(schedule);
    if (observer) {
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      <div
        ...
        className="min-w-0 flex-1 rounded-sm bg-muted p-[2px] shadow-inner"
      >
        <motion.div
          className="h-[3px] origin-left rounded-[2px] bg-[var(--ring)]"
          style={{ width: "100%" }}
          initial={false}
