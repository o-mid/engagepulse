...
const TONES = {
  ...
  caution: {
    bar: "bg-[var(--warning,oklch(0.72_0.15_65))]",
    text: "text-[var(--warning,oklch(0.72_0.15_65))]",
  },
  safe: {
    bar: "bg-[var(--success,oklch(0.63_0.15_155))]",
    text: "text-[var(--success,oklch(0.63_0.15_155))]",
  },
} as const;
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      <div className="mt-2 flex h-5 items-center justify-between gap-3">
        ...
        <motion.span
          aria-hidden
          className="whitespace-nowrap text-[11.5px] leading-5 text-[var(--warning,oklch(0.72_0.15_65))]"
          initial={false}
        ...
        <ul className="mt-3 grid gap-1.5">
          {evaluated.map((rule) => (
            <li key={rule.id} className="flex items-center gap-2">
              <span className="relative grid size-[14px] shrink-0 place-items-center rounded-sm border border-border text-primary-foreground">
                <motion.span
                  className="absolute inset-0 rounded-sm bg-[var(--success,oklch(0.63_0.15_155))]"
                  initial={false}
